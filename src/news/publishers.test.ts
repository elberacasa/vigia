...
const nameKey = (name: string) =>
	fold(name)
		.replace(/\(youtube\)/g, "")
		.replace(/[^a-z0-9]+/g, "");
/** The publisher's site; a YouTube channel page says nothing about who publishes it. */
const siteKey = (url: string) => {
	const host = new URL(url).hostname.replace(/^www\./, "");
	return /(^|\.)youtube\.com$|^youtu\.be$/.test(host) ? null : host;
};

test("review 4 M4: feeds of one outlet (same id but the yt- prefix, same name, same site) declare one publisher", () => {
	const groups = new Map<string, string[]>();
	const add = (key: string, id: string) => groups.set(key, [...(groups.get(key) ?? []), id]);
	for (const o of OUTLETS) {
		add(`id ${o.id.replace(/^yt-/, "")}`, o.id);
